...
#include <ctype.h>	// for islower, toupper, isupper, tolower
...
/* Return s after deleting len chars beginning at pos */
{
	...
	if ((pos < 0) || (len <= 0) || ((i = strlen(s)) <= pos))
		return s;
	if (i < pos + len)
		s[pos] = '\0';
	else
		memmove(s + pos, s + pos + len, i - pos - len + 1);
	return s;
}
/*--------------------------------------------------------------------*/
...
char *ChangeExt(char *NomFich, char *Ext)
{
	...
	if ((s = strrchr(NewName, '.')) && !strchr(s, dirchar))
		strcpy(s, Ext);
	else
		strcat(NewName, Ext);
