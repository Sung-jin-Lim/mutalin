...
static int SaveWeights(t_Weight **Save, t_pTabSeq Sequence, t_indN NbSeq, t_indN SeqDejaAligne)
{
	...
	t_Weight *w, w1, w2;
	t_pTabSeq Seq;
	if ((*Save = (t_Weight *)calloc(sizeof(t_Weight), NbSeq)) == NULL)
		return 0;
	for (i = 1, Seq = Sequence + 1, w1 = 0; i < SeqDejaAligne; i++, Seq++)
		w1 += Seq->Weight;
	for (w2 = 0; i <= NbSeq; i++, Seq++)
		w2 += Seq->Weight;
	if (w1 > 0)
		for (i = 1, Seq = Sequence + 1, w = *Save; i < SeqDejaAligne; i++, w++, Seq++)
			*w = Div(Seq->Weight, 100 * (SeqDejaAligne - 1), w1);
	if (w2 > 0)
		for (i = SeqDejaAligne, Seq = Sequence + i, w = *Save + i - 1; i <= NbSeq; i++, w++, Seq++)
														...
														int SeqDejaAligne, t_ClassHier TabArbre, t_pDescripteurGroupe DG)
{
	...
	if (!Align2Groups(SeqDejaAligne, TabArbre, Sequences, DG, Param))
		return 0;
	if ((SeqDejaAligne != 2) && (SeqDejaAligne != Sequences->NbSeq))
		return 1;
	...
	{
		if (special == 2)
			GapInsert(&Sequences->Sequence[1], &Sequences->Sequence[Neighbour]);
		PESetOutputOrder();
	...
	{
		if (special == 2)
			GapInsert(&Sequences->Sequence[Sequences->NbSeq], &Sequences->Sequence[Neighbour]);
		PESetOutputOrder();
											...
											int SeqDejaAligne)
{
	...
	if (!AlloueTabArbre(TabArbre, Sequences->NbSeq))
		goto fin;
	if (!AlloueTabDessin(TabDessin, Sequences->NbSeq))
		goto fin;

	/*-- allocation et initialisation des tableaux Groupe et NoGroupe ----*/
	if (!InitDescripteurGroupe(&DG, Sequences->NbSeq))
		goto fin;
...
#endif
	/*-- Clustering */
	if (!(special = GetArbre(Sequences, TabArbre[2], TabDessin[2])))
		goto fin;
...
#ifndef clus2dom
			if (OK)
				SauvArbre(TabArbre[2], TabDessin[2], Sequences, ".clu");
#endif
		}
		if (!OK)
			goto fin;
...
affichseul:
	if (PEIsOutputOrder() &&
			((Sequences->NoSeq = (t_indN *)calloc(Sequences->NbSeq + 1, sizeof(t_indN))) != NULL))
		MetOrdre(TabArbre[1], Sequences);
