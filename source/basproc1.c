...
/* initialisation des groupes, de Bon, de Sup */
{
	...
	*Sup = 0;
	...
	for (i = 0, g = Groupe; i <= NbSeq; i++, g++)
		*g = 1;

	memset(Arbre, 0, (NbSeq + 1) * sizeof(t_Branche));
	memset(Dessin, 0, (NbSeq + 1) * sizeof(t_Dessin));
	/* initialiser Noeud pour y ranger les scores */ /* modif */
	for (i = 1, d = Dessin + 1; i <= NbSeq; i++, d++)
		d->Noeud = i;
}
	...
	Sortie : Arbre  et Dessin decrivent le nouvel arbre	*/
{
	...
	if (NbSeq == 2)
		Dessin[2].Col = DebutGraphe;
	else if (Score)
		for (Som = 1, s = Score[NbSeq - 2], d = Dessin + 2; Som < NbSeq; Som++, s++, d++)
			d->Col = DebutGraphe +
					 ...
					 t_indN Prot2)
{
	...
	*x = 0;
	...
	if ((Param->GapExtT % 2) && ((Fin = max(Sequence[Prot].DerCol, Sequence[Prot2].DerCol) - Fin) > 0))
					 ...
					 t_indN Prot2)
{
	...
	for (i = Debut, s1 = Sequence[Prot].Seq + i, s2 = Sequence[Prot2].Seq + i; i <= Fin;
			 i++, s1++, s2++)
		if ((s1->Symb == s2->Symb) && (s1->Symb >= PremLettre))
			Count++;
												...
												float *ScoreTotal)
{
	...
	*ScoreTotal = 0;
	...
	{
		if (Param->ScMethod == 3)
			Sc2a2(Param, DS->Sequence, &sci, Prot, Prot);
				...
				p->Seq a pu etre modifie */
{
	...
	if (Ancien = p->Seq)
		p->Seq = (t_pSeq)realloc(p->Seq, Buffer * sizeof(t_Seq));
	else
		p->Seq = (t_pSeq)malloc(Buffer * sizeof(t_Seq));
	if (!(p->Seq))
	{
...
void MajWeightConsens(t_pLigneSeq Sequence, t_indN NbSeq)
{
	...
	for (i = 0, w = 0, tmp = Sequence + 1; i < NbSeq; i++, tmp++)
		w += tmp->Weight;
	if (w == 0)
	{
		for (i = 0, tmp = Sequence + 1; i < NbSeq; i++, tmp++)
			tmp->Weight = 1;
									...
									ProcAction Action)
{
	...
	while (PremPile != 0)
	{
		...
		while (Pere != PremPile)
		{
			...
			if (!Action(PPere, PremPile, NbSeq, Groupe, Arbre, V))
				return faux;
			PremPile = PPere;
		}
		PremPile = Courant;
