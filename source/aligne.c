...
typedef void Ajoutefunc(t_score Score, t_indN Nb2, t_score *TabTotCoeff, t_Seq **Adresse,
		 ...
		 TabSequence donne les sequences alignees par groupes initiaux   */
{
	...
	for (i = 1; i; i = Prot)
	{
		...
		while ((Longueur > Lon) && (DerPos < Longueur))
		{
			...
			while ((DerPos < Longueur) && !ToutBlanc(DerPos, Nb, i, DS->Sequence, Arbre, ParamAlign))
				DerPos++;
...
/* 16/07/98 TabTotCoeff[0] est pond�r� par WeightM/InSeq */
...
/* 08/12/98 ponderation par WeightM */
{
	...
	for (r = 0, t = TabTotCoeff; r <= Param->DerLettre; r++, t++)
		*t = 0;
	...
	if (WeightM != InSeq)
		for (r = 1, t = TabTotCoeff + 1; r <= Param->DerLettre; r++, t++)
									...
									t_indL *NbEnrg)
{
	...
	Temp = SCO_MIN + DGap;
	for (i = 0, T = GrdTab->Mu; i <= DG->N1 + 1; i++, T++)
		*T = Temp;
	...
	*/
	...
	else
	{
		...
		for (Ns = 0, P = Profile; Ns < Last; Ns++, P++)
			*P += DG->N1 + 1;
		Pos = DG->N;
		T = GrdTab->X + j;
			 ...
			 i > 0; i--, ii--, Direci--, GapExt += DGap2 /*Div(DGap2,TabTotCoeff[0],WeightM)*/)
	{
		...
		{
			*NbEnrg = DG->M1 - 1 - ii;
			if (InitSwapD(*NbEnrg, DG->M1, LineSize) == SWAPIMPOSSIBLE)
			{
				TraiteErr(8);
				return faux;
			}
...
static int AlloueProfile(t_Weight ***pProfile, int NbL, int NbC)
{
	...
	for (i = 0, p = *pProfile; i < NbL; i++, p++)
		if ((*p = (t_Weight *)calloc(NbC, sizeof(t_Weight))) == NULL)
			return 0;
	return 1;
}
...
{
	int j;
	for (j = 0; j < N; j++, i = Arbre[i].SeqSuiv)
		AffichAllume(i, Col);
}
...

{
	...
	{
		if (Mult)
			OK = Monter(DG->G2, DS->Sequence, Arbre, Param, GrdTab, DG,
									Adresse, Profile, WeightN, /*RDGap,*/ TabTotCoeff, Direction, &Saut, &NbEnrg);
		else
			OK = Monter2(DG->G2, DS->Sequence, Arbre, Param, GrdTab, DG,
...
- Supprime Groupe2 dans Groupe  */
{
	...
	if (init) /*09/05/94 pour seq.partielles*/
	{
		...
		if ((DG->M - DG->M1 != 1) && (DG->N - DG->N1 != 1))
			ParamAlign->GapExtT++;
		if ((DG->M < DS->Sequence[DG->G1].Taille) && (DG->N < DS->Sequence[DG->G2].Taille))
			ParamAlign->GapExtT += 2;
	}
	else
	{
													...
													t_pTabSeq Sequence)
{
	...
	t_indN k, n;
	float s;
	for (n = 0, k = i, tot = 0; n < N; k = Arbre[k].SeqSuiv, n++)
		tot += Sequence[k].Weight;
	if (!tot)
...
*/
{
	...
	for (i = 0, g = DG->Groupe; i <= DS->NbSeq; i++, g++)
		*g = 1;
	...
	if (!Param->Weighted)
	{
		if (N * N > T1)
		{
			fprintf(stderr, "ss poids\n");
			TraiteErr(9);
			return faux;
		}
		for (n = 0, tmp = DS->Sequence; n <= N; n++, tmp++)
			tmp->Weight = 1;
	...
	if ((N * N * (Dessin[2].Col - Dessin[0].Col) > T1) && (Dessin[2].Col > Dessin[0].Col))
	{
		...
		if (T1 > 0)
			for (n = 0; n <= N; n++)
				Dessin[n].Col = Div(1.0 * Dessin[n].Col, T1, T2);
		else
			for (n = 0; n <= N; n++)
				Dessin[n].Col = 1;
	...
	if ((noeud = (t_indN *)(calloc(N + 1, sizeof(t_indN)))) == NULL)
		return faux;
	for (n = 0, i = Dessin[0].Col; n <= N; n++)
		noeud[n] = i;
	for (n = 0, tmp = DS->Sequence; n <= N; n++, tmp++)
		tmp->Weight = 0;
	...
	for (n = N, des = Dessin + n; n > 1; n--, des--)
	{
		...
		Ni = g[i];
		Nj = g[j];
		if (!ActAligner(i, j, N, g, Arbre, &Data))
		{
			free(noeud);
			return faux;
		}
	...
	G3 --+       AG3 --+	*/
{
	...
	Data->Bon[Groupe1] = Data->Bon[Groupe1] && Data->Bon[Groupe2] && (Data->AncArbre[Groupe1].SeqSuiv == Groupe2) && (Data->AncArbre[Groupe2].Pere == Groupe1) && (Data->AncArbre[AncGroupe3].Pere != Groupe2);
	if (Groupe1 != 0)
	{
		if (Data->Bon[Groupe1])
		{
...
static int ArbEgal(t_ClassHier A1, t_ClassHier A2, t_indN NbSeq)
{
	...
	for (j = 1; (j <= NbSeq) && (A1[j].Pere == A2[j].Pere) && (A1[j].SeqSuiv == A2[j].SeqSuiv); j++)
		;
	return (j > NbSeq);
}
	...
	if (Nb1 && Nb2)
	{
		...
		for (Pos = 1; Pos <= Data->Sequence[0].DerCol; Pos++)
		{
			...
			if (!(Vide && Vide2))
			{
				...
				{
					if (Ferme)
						sc -= (Data->Param->Gap + g2 * Data->Param->Gap2) * GapLoc;
					Ferme = vrai;
					g2 = 0;
				}
 ...
 */
{
	...
	Ancien = TabArbre[0];
	for (i = 0; i < 2; i++)
		TabArbre[i] = TabArbre[i + 1];
	TabArbre[2] = Ancien;
	memset(TabArbre[2], 0, (DS->NbSeq + 1) * sizeof(t_Branche));
	Ancien = TabDessin[0];
	for (i = 0; i < 2; i++)
		TabDessin[i] = TabDessin[i + 1];
	TabDessin[2] = Ancien;
										...
										t_DessinArb *TabDessin)
{
	...
	for (i = 0, Seq = DS->Sequence; (i <= DS->NbSeq) && OK; i++, Seq++)
	{
		...
		if (Seq->BuffLen == 0)
			...
			OldBufflen = (Seq->Taille | 63) + 1;

		Seq->BuffLen = NewBufflen;
		Seq->Seq = NewSeq;
		if (NewSeq && OldBufflen)
		{
			if ((OldBufflen == NewBufflen) || (Seq->Seq = realloc(NewSeq, OldBufflen)))
		...
		{
			if (Seq->Seq = malloc(OldBufflen))
				Seq->BuffLen = OldBufflen;
			else
				Seq->BuffLen = OldBufflen = 0;
		}
		else
		{
			if (NewSeq)
		...
		if (OldBufflen && ((Seq->Taille > Seq->BuffLen) ||
											 (!fread(Seq->Seq + 1, 1, Seq->Taille, f))))
			OK = faux;
	}
	fclose(f);
	if (!OK)
	...
	tmp = TabArbre[2];
	for (i = 1; i >= 0; i--)
		TabArbre[i + 1] = TabArbre[i];
	TabArbre[0] = tmp;
	tmp = TabDessin[2];
	for (i = 1; i >= 0; i--)
		TabDessin[i + 1] = TabDessin[i];
	TabDessin[0] = tmp;
	...
	/*7/7/94 TabArbre[2]->TabArbre[1]->TabArbre[0] */
	{
		...
		*/
		if (OK = Alignement(TabArbre[1], Dessin[1], DG, DS, Param))
		{
			...
			recup = Recupere(DS, TabArbre, Dessin);
	}
								 ...
								 t_pDescripteurGroupe DG, t_pParamAlign Param)
{
	...
	for (i = 1, wm = 0; i <= N2; i++)
		wm += DS->Sequence[i].Weight;
	wm = max(N2 - 1, wm);
	for (wn = 0; i <= N; i++)
		wn += DS->Sequence[i].Weight;
	wn = max(N - N2 + 1, wn);
	if (wm * wn > T1)
	{
		s = sqrt(1.0 * T1 / wm / wn);
